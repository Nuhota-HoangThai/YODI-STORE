<template>
  ...
  >
    <banner-product />
    <div
      class="container mx-auto text-black px-14 py-3"
      v-if="products.length > 0"
    >
      <!-- Title -->
      <h1 class="text-xl font-semibold text-center py-1">TẤT CẢ SẢN PHẨM</h1>
      <div class="border-b-2 border-black"></div>

      <!-- Filter -->
      <form action="" class="flex gap-4 items-center mb-4 mt-2 justify-end">
        <div class="row flex items-center gap-2">
          <span>Giá: </span>
          <select
            :value="priceRef"
            @change="onSortPrice($event)"
            class="bg-gray-50 border-gray-800 text-gray-900 text-sm rounded-sm py-1"
          >
            <option selected value="1">Tăng dần</option>
      ...
      <!-- Product -->
      <div class="grid grid-cols-4" v-if="products.length > 0">
    ...
    <div class="my-8 text-center text-white" v-else>KHÔNG CÓ SẢN PHẨM</div>
  </div>
</template>

<script>
import BannerProduct from "@/components/Product/BannerProduct.vue";
...
export default {
  components: { CardProduct, BannerProduct },
  setup() {
    // Hooks
    ...
    //nav button
    // const searchText = ref(route.query.title || "Casio");
    // const casioClick = (searchText) => {
